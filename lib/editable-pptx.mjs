...
export async function captureEditablePptx(config) {
  ...
  const downloadDir = fs.mkdtempSync(path.join(os.tmpdir(), 'make-pdf-'));
  ...
  const bundlePath = path.join(__dirname, '../node_modules/dom-to-pptx/dist/dom-to-pptx.bundle.js');

  if (!fs.existsSync(bundlePath)) {
    throw new Error('dom-to-pptx bundle not found. Run: npm install dom-to-pptx');
  }
  ...
  // We'll store the HTML state of each slide, then export all at once
  const slideHTMLs = [];
  ...
  console.log(`\nExporting ${slideHTMLs.length} slides to editable PPTX...`);
  ...
  return { output, slideCount: slideHTMLs.length };
}
