...
export async function capturePDF(config) {
  ...
  if (format === 'pptx-image') {
    ...
    console.log('\nGenerating PDF...');
    await generatePDF(screenshots, output, viewport, sandbox);
  }
  ...
  if (!keepScreenshots) {
    ...
    try {
      // Only remove if empty (don't use recursive to avoid deleting user files)
...
async function generatePDF(screenshots, output, viewport, sandbox = false) {
  ...
  await createPDFWithPuppeteer(screenshots, output, viewport, sandbox);
}

async function createPDFWithPuppeteer(screenshots, output, viewport, sandbox = false) {
  const browserArgs = sandbox ? [] : ['--no-sandbox', '--disable-setuid-sandbox'];
  const browser = await puppeteer.launch({
    headless: true,
    args: browserArgs
